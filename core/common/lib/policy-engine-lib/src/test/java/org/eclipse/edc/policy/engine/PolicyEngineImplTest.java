...
class PolicyEngineImplTest {
    ...
    class ForContext {
        ...
        @Test
        void shouldGiveScopedPolicyEngine() {
            var context = new TestContext("data");
            var engine = policyEngine.forScope(new PolicyScope<TestContext>("test-scope"));

            assertThat(engine).isNotNull();

            var result = engine.evaluate(Policy.Builder.newInstance().build(), context);
            assertThat(result).isSucceeded();
        }

        @Test
        void shouldKeepSingleInstanceForSameScope() {
            var first = policyEngine.forScope(new PolicyScope<>("test-scope"));
            var second = policyEngine.forScope(new PolicyScope<>("test-scope"));
    ...
    class Evaluate {
        ...
        void validateUnsatisfiedDuty() {
            ...
            policyEngine.registerFunction(TEST_SCOPE, Duty.class, "foo", (op, rv, duty, ctx) -> false);
        ...
        void validateUngrantedPermission() {
            ...
            policyEngine.registerFunction(TEST_SCOPE, Permission.class, "foo", (op, rv, duty, context) -> false);
            var context = PolicyContextImpl.Builder.newInstance().build();
        ...
        void validateTriggeredProhibition() {
            ...
            policyEngine.registerFunction(TEST_SCOPE, Prohibition.class, "foo", (op, rv, duty, context) -> true);
            var context = PolicyContextImpl.Builder.newInstance().build();
        ...
        void shouldNotTriggerDynamicFunction_whenBindAlreadyAvailable(Policy policy, Class<Rule> ruleClass) {
            bindingRegistry.bind("foo", ALL_SCOPES);
            policyEngine.registerFunction(TEST_SCOPE, ruleClass, "foo", (op, rv, duty, context) -> !ruleClass.isAssignableFrom(Prohibition.class));
            bindingRegistry.dynamicBind((key) -> Set.of(TEST_SCOPE));
